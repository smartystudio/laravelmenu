    ...
    <div class="card-header">
        ...
        <div class="card-link float-right" data-toggle="collapse" href="#collapse{{ $m['id'] }}">
            <span class="item-controls"> 
                <span class="item-type">{{ __('Link') }} <i class="fa fa-angle-down narrow-icon" aria-hidden="true"></i></span>
            </span>
        </div>
    </div>
    {{-- @if($key == 0) show @endif --}}
    <div id="collapse{{$m['id']}}" class="collapse " data-parent="#accordion">
        <div class="card-body">
            <div class="menu-item-settings" id="menu-item-settings-{{ $m['id'] }}">
                ...
                <div class="form-group">
                    <label for="">{{ __('CSS class (optional)') }}</label>
                ...
                @if (!empty($roles))
                    <div class="form-group">
                        <label for="edit-menu-item-role-{{ $m['id'] }}">{{ __('Role') }}</label>
                ...
                <div class="form-group">
                    <button type="button" onclick="deleteItem({{ $m['id'] }})" class="btn btn-danger btn-sm" id="delete-{{ $m['id'] }}" href="javascript:void(0)">{{ __('Delete') }}</button>
                    <button type="button" onclick="updateItem({{ $m['id'] }})" class="btn btn-primary btn-sm" id="update-{{ $m['id'] }}" href="javascript:void(0)">{{ __('Update') }}</button>
                </div>
            </div>
        </div>
    </div>
    @if (isset($m['child']) && count($m['child']) > 0)
        <ol class="dd-list">
            @foreach($m['child'] as $_m)
                @include('menu::partials.loop-item', ['m' => $_m, 'key' => 1])
            @endforeach
        </ol>
    @endif
</li>
