            ...
            <div class="col-md-12">
                <form class="form-inline" action="" method="post">
                    <div class="form-group">
                        <label for="email" class="mr-sm-2">{{ __('Name') }}</label>
                        <input name="menu-name" id="menu-name" type="text" class="form-control menu-name regular-text menu-item-textbox" title="Enter menu name" value="@if (isset($indmenu)){{ $indmenu->name }} @endif">
                        @if(request()->has('action'))
                            <button type="button" onclick="createNewMenu()" name="save_menu" class="btn btn-primary menu-save ml-2">{{ __('Create Menu') }}</button>
                        @elseif(request()->has('menu'))
                            <button type="button" onclick="actualizarMenu(false)" name="save_menu" class="btn btn-primary menu-save ml-2">{{ __('Save Menu') }}</button>
                        @else
                            <button type="button" onclick="createNewMenu()" name="save_menu" class="btn btn-primary menu-save ml-2">{{ __('Create Menu') }}</button>
                        @endif
                    </div>
                ...
                @elseif (request()->get('menu') == 0)
                    <div class="jumbotron jumbotron-fluid p-2">
                        <div class="container">
                            ...
                            <p class="lead">{{ __('Please enter the name and select \'Create Menu\' button') }}</p>
                        </div>
                    </div>
                @else
                    <div class="jumbotron jumbotron-fluid p-2">
                        <div class="container">
                            <h3>{{ __('Create Menu Item') }}</h3>
                            <p class="lead"></p>
                        </div>
                    </div>
                @endif
